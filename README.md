...
- **多轨与动画（v1.3.0 新增）**：
  - **多视频轨**：最多 4 行（V1 底层 → V4 顶层），行级锁定/隐藏，拖拽素材到指定行，行内防重叠
  - **画中画**：上排片段默认缩小居中，位置/缩放/旋转可调，预览画布直接拖拽
  - **关键帧动画**：位置/缩放/不透明度/旋转线性插值，播放头添加、菱形标记、跳转/选中/删除
  - **转场库**：叠化（取上段末帧）、划像左右、圆形展开、上滑/下滑、缩放推入、黑场淡入，时长可调（V1 层片段间）
- **导出（v1.2.0）**：
  - **MP4 帧精确导出**（推荐）：WebCodecs VideoEncoder 逐帧 H.264 编码，多轨/画中画/关键帧/转场全部参与合成
  - 音频：decodeAudioData → OfflineAudioContext 混音（音量/静音/变速/淡入淡出全部还原）→ AudioEncoder AAC
  - **自研最小 MP4 muxer**（ftyp+mdat+moov，avcC/esds 手写盒，保持零依赖）
